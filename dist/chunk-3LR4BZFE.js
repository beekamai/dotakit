...
var watchResultName = (result) => result === null ? "timeout (no GC response)" : CMsgSpectateFriendGameResponse_EWatchLiveResult[result] ?? `result ${result}`;
var Live = class {
  gc;
  timeoutMs;
  logger;
  ...
  async spectateFriend(steamId64, options = {}) {
    const reply = await oneShot(this.gc, {
      request: [7073 /* k_EMsgGCSpectateFriendGame */, { steamId: steamId64, live: options.live ?? true }],
